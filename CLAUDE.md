...
- After every edit, deploy and rebuild VMZ:
  ```
  cp "VR Mod/resources/vr_mod_init.gd" "C:/Program Files (x86)/Steam/steamapps/common/Road to Vostok/vr_mod_init.gd"
  # then rebuild VMZ and copy to mods/ (see build.bat)
  ```
...
2. **Deploy after every change** — deploy `vr_mod_init.gd` to game root AND rebuild/redeploy the VMZ to `mods/`.
